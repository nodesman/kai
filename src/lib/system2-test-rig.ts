...
describe('Calculator.subtract', () => {
  it('should correctly subtract two numbers', () => {
    ...
    expect(calculator.subtract(5, 2)).toBe(3);
        ...
        `;
      } else if (scenario?.description === "should correctly multiply two numbers") {
        console.log(`ConfigurableAI: Generating specific test for "should correctly multiply two numbers"`);
        return `
// Test generated by ConfigurableSimpleAiModelService for Calculator.multiply
import { Calculator } from './calculator'; 
...
describe('Calculator.multiply', () => {
  it('should correctly multiply two numbers', () => {
    ...
    expect(calculator.multiply(-5, 2)).toBe(-10);
      ...
      `;
    } else if (scenario?.description === "should correctly divide two numbers" || scenario?.description === "should throw an error when dividing by zero") {
      console.log(`ConfigurableAI: Generating specific test for "${scenario.description}"`);
      if (scenario.description === "should correctly divide two numbers") {
        return `
// Test generated by ConfigurableSimpleAiModelService for Calculator.divide (happy path)
import { Calculator } from './calculator'; 
...
describe('Calculator.divide', () => {
  it('should correctly divide two numbers', () => {
    ...
    expect(calculator.divide(0, 5)).toBe(0);
        ...
        `;
      } else { // should throw an error when dividing by zero
        return `
// Test generated by ConfigurableSimpleAiModelService for Calculator.divide (error case)
import { Calculator } from './calculator'; 
...
Focus on implementing the 'subtract' method in '${CALCULATOR_FILE_PATH}'.`; // Using the robust CALCULATOR_FILE_PATH
      } else if (errorDetails?.testName?.includes("should correctly multiply two numbers")) {
        console.log(`ConfigurableAI: Diagnosing specific failure for "should correctly multiply two numbers"`);
        return `ConfigurableAI Diagnosis: The test 'Calculator.multiply > should correctly multiply two numbers' failed.
Error: ${errorDetails?.errorMessage || 'Method not found or incorrect implementation.'}.
Likely cause: The 'multiply' method is missing from the Calculator class or not implemented correctly.
File: ${errorDetails?.filePath || CALCULATOR_FILE_PATH}.
Focus on implementing the 'multiply' method in '${CALCULATOR_FILE_PATH}'.`;
    } else if (errorDetails?.testName?.includes("Calculator.divide")) { // Match any test from Calculator.divide suite
      console.log(`ConfigurableAI: Diagnosing specific failure for "Calculator.divide" tests`);
      let specificCause = "The 'divide' method is missing, not implemented correctly, or does not handle an edge case properly.";
      if (errorDetails?.testName?.includes("should throw an error when dividing by zero") && !errorDetails?.errorMessage?.includes("Division by zero")) {
        specificCause = "The 'divide' method exists but does not correctly throw an error for division by zero, or throws the wrong error message.";
      } else if (errorDetails?.testName?.includes("should throw an error when dividing by zero")) {
        specificCause = "The 'divide' method seems to be attempting to handle division by zero, but the test expectation might be wrong or the error handling is not fully correct.";
      } else if (errorDetails?.errorMessage?.includes("is not a function")) {
        specificCause = "The 'divide' method is likely missing from the Calculator class.";
      }
      return `ConfigurableAI Diagnosis: The test '${errorDetails?.testName}' failed.
Error: ${errorDetails?.errorMessage || 'Method not found or incorrect implementation.'}.
Likely cause: ${specificCause}
File: ${errorDetails?.filePath || CALCULATOR_FILE_PATH}.
Focus on implementing or correcting the 'divide' method in '${CALCULATOR_FILE_PATH}', paying attention to division by zero.`;
    }
...
export class Calculator {
  ...
  subtract(a: number, b: number): number {
    return a - b; // The fix
      ...
      } else if (context?.scenario?.description === "should correctly multiply two numbers") {
        console.log(`ConfigurableAI: Generating specific code fix for "should correctly multiply two numbers"`);
        const codeWithAddSubtractAndMultiply = `// src/lib/sample-project/calculator.ts
export class Calculator {
  add(a: number, b: number): number {
    return a + b;
  }
  ...
  multiply(a: number, b: number): number {
    return a * b; // The fix for multiply
  }
}`;
        return JSON.stringify({
          filePathToModify: CALCULATOR_FILE_PATH,
          codeOrDiffContent: codeWithAddSubtractAndMultiply,
          type: 'full_content'
        });
    } else if (context?.scenario?.description === "should correctly divide two numbers" || context?.scenario?.description === "should throw an error when dividing by zero") {
      console.log(`ConfigurableAI: Generating specific code fix for "Calculator.divide" scenarios`);
      // This code fix assumes add, subtract, and multiply might already be present.
      const codeWithAllMethods = `// src/lib/sample-project/calculator.ts
export class Calculator {
  add(a: number, b: number): number {
    return a + b;
  }
  ...
  divide(a: number, b: number): number {
    ...
    return a / b; // The fix for divide
        ...
        return JSON.stringify({
          filePathToModify: CALCULATOR_FILE_PATH,
          codeOrDiffContent: codeWithAllMethods,
          type: 'full_content'
        });
    }
    }
