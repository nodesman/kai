...
class Gemini2ProModel extends BaseModel {
    ...
    constructor(config: Config) {
        ...
        this.modelName = config.gemini.model_name || "gemini-2.5-pro-preview-03-25"; // Default to Pro (UPDATED)
    ...
    // --- getResponseFromAI (for Chat - Unchanged from previous state) ---
    ...
    // --- queryGeminiChat (Helper for getResponseFromAI - Unchanged from previous state) ---
    ...
    async generateContent(request: GenerateContentRequest): Promise<GenerateContentResult> {
        ...
        while (attempts <= this.maxRetries) {
            try {
                ...
                const errorCode = (error as any).code || 'UNKNOWN'; // Get error code if attached by handleError
