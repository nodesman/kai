...
void ChatInterface::setupUI() {
    ...
    mainLayout->setSpacing(0); // Remove spacing, splitter handles it
    ...
    // --- Use QSplitter ---
    QSplitter *splitter = new QSplitter(Qt::Vertical, this); // Vertical orientation
    splitter->addWidget(conversationHistory);
    splitter->addWidget(inputContainer);
    splitter->setStretchFactor(0, 3);    // Initial stretch: history gets 3/4, input 1/4
    splitter->setStretchFactor(1, 1);
    splitter->setChildrenCollapsible(false); // Prevent collapsing

    mainLayout->addWidget(splitter); // Add the splitter to the main layout
